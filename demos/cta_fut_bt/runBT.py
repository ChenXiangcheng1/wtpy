...
# sys.path.append('E://dev-projects/git_repos/wtpy/demos/Strategies')
# sys.path.append('./demos/Strategies')
from DualThrust import StraDualThrust

# from demos.Strategies.DualThrust import StraDualThrust

# from ..DualThrust.StraDualThrust import Strategies.DualThrust.StraDualThrust
...
if __name__ == '__main__':
    ...
    engine.init('../common/', 'configbt.yaml')  # 配置
    engine.configBacktest(201909100930, 201912011500)  # 时间
    engine.configBTStorage(mode='csv', path='../storage/')  # 数据

    # 注册自定义连续合约规则
    # engine.registerCustomRule(ruleTag="0001", filename="../common/hots.json")

    engine.commitBTConfig()  # 提交回测配置
